...
function m(e) {
	...
	function o(r) {
		...
		let i = l(r.rockSpeed ?? 0, 0, 4.5), a = l(r.ascentProgress ?? 0, 0, 1), o = !!r.isPushing, s = l(r.moodImpact ?? 0, 0, 1), c = l((i - .08) / 1.9, 0, 1), u = l((i - .08) / 1.55, 0, 1), d = o ? 1 : 0, f = l(e.masterGain * (.92 + d * .16 + s * .08), 0, 1), p = e.windGain * (.62 + a * .56) * (.9 + c * .34) * (o ? 1.08 : .92), m = e.frictionGain * ((o ? .3 : .06) + u * (o ? .9 : .44)), h = e.breathGain * (o ? .72 + u * .5 : .11), g = e.enableSampleLayer && t.samplesReady ? e.sampleBlend : 0, _ = 1 - g;
		n(t.masterGainNode, f, .24), n(t.windGainNode, p * _, .18), n(t.frictionGainNode, m * _, .12), n(t.breathGainNode, h, .16), n(t.windSampleGainNode, p * g, .22), n(t.frictionSampleGainNode, m * g, .16);
		let v = t.context.currentTime;
		t.windHighPassNode.frequency.setTargetAtTime(46 + a * 58, v, .2), t.windLowPassNode.frequency.setTargetAtTime(260 + a * 420 + s * 130, v, .2), t.frictionBandPassNode.frequency.setTargetAtTime(110 + i * 132, v, .12), t.frictionBandPassNode.Q.setTargetAtTime(1.08 + u * 2, v, .12), t.frictionLowPassNode.frequency.setTargetAtTime(430 + i * 210 + (o ? 50 : 0), v, .14), t.breathFilterNode.frequency.setTargetAtTime(170 + (o ? 180 : 90) + u * 110, v, .2), t.breathOsc && t.breathOsc.frequency.setTargetAtTime(90 + u * 38 + (o ? 16 : 0), v, .2);
	}
	function s(n) {
...
function E(e) {
	...
	function k(e) {
		let r = t.getBoundingClientRect(), i = O(e), a = (i.clientX - r.left) / r.width, o = (i.clientY - r.top) / r.height, s = n.bounds;
		return {
	...
	function A(e) {
		let t = i.bounds;
		return e.x >= t.min.x - 24 && e.x <= t.max.x + 24 && e.y >= t.min.y - 24 && e.y <= t.max.y + 24;
	}
	function te(e) {
		p() || o.active || (t.style.cursor = ee([i], e) || A(e) ? "grab" : "default");
	}
	function j(e) {
		if (!o.active || p()) return;
		e.preventDefault();
		let n = O(e), r = n.clientX - o.startClientX, i = n.clientY - o.startClientY;
		if (!o.moved && Math.hypot(r, i) < d) return;
		o.moved = !0, E = 0, v(u.FREE_PAN);
	...
	function M(e) {
		e.preventDefault(), s.unlock();
		let n = k(e), r = ee([i], n) || A(n), o = h(), c = o === l.PLAYING || o === l.WAITING;
		if (r && c) {
			if (o === l.WAITING && i.render.opacity < .5) return;
			o === l.WAITING && g(l.PLAYING), E = 0, x(), v(u.FOCUS_SISYPHUS), y(), m(!0), t.style.cursor = "grabbing";
			return;
		}
		let d = O(e), f = Date.now(), p = Math.hypot(d.clientX - D.x, d.clientY - D.y);
		if (E > 0 && f - E < 320 && p < 26) {
			E = 0, x(), C(), t.style.cursor = "default";
			return;
		}
		E = f, D = {
			x: d.clientX,
			y: d.clientY
		}, b(d.clientX, d.clientY), S(a.x, a.y, _(window.innerHeight > window.innerWidth));
	}
	function N(e) {
		e.preventDefault(), m(!1), x(), te(k(e));
	}
	function P(e) {
		if (o.active) {
			j(e);
	...
	function F(e) {
		...
		if (t === "q") {
			let t;
			e.shiftKey ? (c.clearForcedTier(), t = `${c.getSnapshot().tier.toUpperCase()} (AUTO)`) : t = `${c.cycleTier().toUpperCase()} (BLOQUEADA)`, T(performance.now() + 1400), console.log(`%cCalidad visual: ${t}`, "color:#9bd1ff;font-weight:bold");
		}
	}
	function I() {
		t.addEventListener("mousedown", M), t.addEventListener("touchstart", M, { passive: !1 }), t.addEventListener("mousemove", P), t.addEventListener("touchmove", P, { passive: !1 }), t.addEventListener("mouseup", N), t.addEventListener("touchend", N, { passive: !1 }), t.addEventListener("touchcancel", N), t.addEventListener("mouseleave", N), window.addEventListener("keydown", F);
	}
	function L() {
		t.removeEventListener("mousedown", M), t.removeEventListener("touchstart", M), t.removeEventListener("mousemove", P), t.removeEventListener("touchmove", P), t.removeEventListener("mouseup", N), t.removeEventListener("touchend", N), t.removeEventListener("touchcancel", N), t.removeEventListener("mouseleave", N), window.removeEventListener("keydown", F);
	}
	return Object.freeze({
		register: I,
		destroy: L
...
function k(e) {
	...
	function T(e, t, n, r = 1) {
		e.save(), e.globalAlpha = r, w(e, t), e.fillStyle = n, e.fill(), e.restore();
	}
	function E(e, t) {
		e.save();
		for (let n of S) {
			...
			if (e.fillStyle = `rgba(${a}, ${o}, ${s}, ${i.toFixed(3)})`, e.beginPath(), e.arc(n.x, n.y, n.radius, 0, O), e.fill(), n.cross) {
				let t = n.radius * 6;
			...
			if (n.glow) {
				...
				t.addColorStop(0, `rgba(${a}, ${o}, ${s}, ${(i * .22).toFixed(3)})`), t.addColorStop(.5, `rgba(${a}, ${o}, ${s}, ${(i * .06).toFixed(3)})`), t.addColorStop(1, `rgba(${a}, ${o}, ${s}, 0)`), e.fillStyle = t, e.beginPath(), e.arc(n.x, n.y, n.radius * 5.5, 0, O), e.fill();
			}
		}
		e.restore();
	}
	function k(e, n, r, i, a) {
		e.save();
		for (let o of C) {
			let s = Math.sin(a * o.speed + o.phase) * o.drift, c = n * o.parallax, l = e.createLinearGradient(0, o.y, 0, o.y + o.height);
			l.addColorStop(0, "rgba(226, 231, 239, 0)"), l.addColorStop(.45, `rgba(226, 231, 239, ${(o.alpha * r.mistAlphaMultiplier * (1 + i.sceneFx.hazeBoost)).toFixed(3)})`), l.addColorStop(1, "rgba(226, 231, 239, 0)"), e.fillStyle = l, e.fillRect(-1400 + s + c, o.y, t + 2800, o.height);
		}
		e.restore();
	}
	function A(e, r, i, a, o) {
		let s = e.createLinearGradient(0, 0, 0, n);
		s.addColorStop(0, "#111728"), s.addColorStop(.18, "#151c2e"), s.addColorStop(.42, D.skyTop), s.addColorStop(.62, D.skyMid), s.addColorStop(1, D.skyHaze), e.fillStyle = s, e.fillRect(-1e3, 0, t + 2e3, n + 1e3), E(e, o);
		let c = i.moonRadius, l = 1 + a.sceneFx.glowBoost, u = e.createRadialGradient(840, 112, c * .5, 840, 112, 320 * i.moonGlowScale * l);
		u.addColorStop(0, `rgba(240, 216, 162, ${(.06 * i.moonGlowAlpha).toFixed(3)})`), u.addColorStop(.3, `rgba(230, 210, 170, ${(.03 * i.moonGlowAlpha).toFixed(3)})`), u.addColorStop(.7, `rgba(180, 200, 230, ${(.015 * i.moonGlowAlpha).toFixed(3)})`), u.addColorStop(1, "rgba(180, 200, 230, 0)"), e.fillStyle = u, e.fillRect(-1e3, -1e3, t + 2e3, n + 2e3);
		let d = e.createRadialGradient(840, 112, c * .8, 840, 112, 160 * i.moonGlowScale * l);
		d.addColorStop(0, `rgba(248, 228, 180, ${(.22 * i.moonGlowAlpha * l).toFixed(3)})`), d.addColorStop(.35, `rgba(240, 216, 162, ${(.1 * i.moonGlowAlpha).toFixed(3)})`), d.addColorStop(.7, `rgba(240, 216, 162, ${(.03 * i.moonGlowAlpha).toFixed(3)})`), d.addColorStop(1, "rgba(240, 216, 162, 0)"), e.fillStyle = d, e.fillRect(540, -188, 600, 600), e.save(), e.globalAlpha = i.moonAlpha, e.beginPath(), e.arc(840, 112, c, 0, O), e.clip();
		let f = e.createRadialGradient(840 - c * .3, 112 - c * .25, 0, 840, 112, c);
		f.addColorStop(0, "#fae8c4"), f.addColorStop(.5, "#f0d8a2"), f.addColorStop(.85, "#dfc78a"), f.addColorStop(1, "#c8b070"), e.fillStyle = f, e.fillRect(840 - c, 112 - c, c * 2, c * 2), e.globalAlpha = i.moonAlpha * .12, e.fillStyle = "#8a7a52", e.beginPath(), e.ellipse(840 - c * .15, 112 + c * .1, c * .3, c * .2, -.3, 0, O), e.fill(), e.beginPath(), e.ellipse(840 + c * .2, 112 - c * .2, c * .15, c * .18, .4, 0, O), e.fill(), e.globalAlpha = i.moonAlpha * .45;
		let p = c * .65, m = e.createRadialGradient(840 + p, 112 + c * .1, c * .1, 840 + p, 112 + c * .1, c * 1.3);
		m.addColorStop(0, "rgba(15, 20, 35, 0.7)"), m.addColorStop(.5, "rgba(15, 20, 35, 0.3)"), m.addColorStop(1, "rgba(15, 20, 35, 0)"), e.fillStyle = m, e.fillRect(840 - c, 112 - c, c * 2, c * 2), e.restore();
		let h = e.createLinearGradient(0, 240, 0, n);
		if (h.addColorStop(0, "rgba(255, 255, 255, 0)"), h.addColorStop(1, D.haze), e.fillStyle = h, e.fillRect(-1e3, 240, t + 2e3, n), i.hazeAlphaBoost > 0) {
	...
	function te(e, t, r, i) {
		e.save(), e.translate(t * .18 * i.distantParallaxMultiplier, r * .04), T(e, [
			{
				x: -40,
				y: n + 20
			},
		...
		], D.farMountain, i.distantMountainAlpha), T(e, [
			{
				x: -40,
				y: n + 20
			},
		...
		], D.farMountainShadow, .9 * i.distantMountainAlpha), e.restore();
	}
	function j(e) {
		let r = e.createLinearGradient(640, 40, t, n);
		r.addColorStop(0, "#5a6473"), r.addColorStop(.52, "#465061"), r.addColorStop(1, "#2b3442"), T(e, v, r, .98);
		let o = e.createLinearGradient(140, 170, 640, n);
		o.addColorStop(0, D.mountainLight), o.addColorStop(.45, D.mountainBase), o.addColorStop(1, D.mountainShadow), T(e, y, o), e.save(), w(e, y), e.clip();
		let s = e.createLinearGradient(0, n, t * .78, 120);
		s.addColorStop(0, "rgba(26, 31, 41, 0.42)"), s.addColorStop(.55, "rgba(26, 31, 41, 0.12)"), s.addColorStop(1, "rgba(255, 255, 255, 0.08)"), e.fillStyle = s, e.fillRect(-60, 0, t + 120, n + 180);
		let c = e.createRadialGradient(i - 36, a + 18, 12, i - 36, a + 18, 168);
		c.addColorStop(0, "rgba(248, 233, 204, 0.16)"), c.addColorStop(.45, "rgba(248, 233, 204, 0.06)"), c.addColorStop(1, "rgba(248, 233, 204, 0)"), e.fillStyle = c, e.fillRect(i - 220, a - 140, 360, 280), e.restore(), e.strokeStyle = "rgba(222, 228, 236, 0.10)", e.lineWidth = 1.5, w(e, _, !1), e.stroke();
	}
	function M(e) {
		let i = e.createLinearGradient(0, r, 0, n + 1e3);
		i.addColorStop(0, D.groundTop), i.addColorStop(1, D.groundBottom), T(e, b, i), e.strokeStyle = "rgba(228, 233, 240, 0.08)", e.lineWidth = 1, e.beginPath(), e.moveTo(-1e3, r), e.lineTo(t + 1e3, r), e.stroke();
	}
	function N(e, t, n, r, i, a) {
		...
		e.save(), e.translate(t.position.x + 16, o + 10), e.rotate(-.16), e.scale(c, 1 / c), e.fillStyle = `rgba(10, 14, 22, ${u.toFixed(3)})`, e.beginPath(), e.ellipse(0, 0, n + s * .1, r + s * .04, 0, 0, O), e.fill(), e.restore();
	}
	function P(e, r, i) {
		let a = i ? i.fogAlphaMul : 1, o = e.createLinearGradient(0, 172, 0, n + 80);
		o.addColorStop(0, `rgba(236, 228, 208, ${(r.fog.nearAlpha * .4 * a).toFixed(3)})`), o.addColorStop(r.fog.depthBias, `rgba(228, 217, 196, ${(r.fog.farAlpha * .55 * a).toFixed(3)})`), o.addColorStop(1, `rgba(30, 37, 52, ${(r.fog.farAlpha * a).toFixed(3)})`), e.fillStyle = o, e.fillRect(-1200, 170, t + 2400, n);
	}
	function F(e, t) {
		e.save();
		for (let n of t) {
			...
			r.addColorStop(0, `rgba(244, 232, 210, ${t.toFixed(3)})`), r.addColorStop(1, "rgba(244, 232, 210, 0)"), e.fillStyle = r, e.beginPath(), e.arc(n.x, n.y, n.radius, 0, O), e.fill();
		}
		e.restore();
	}
	function I(e, t) {
		let { viewMinX: n, viewMinY: r, visualProfile: i, layerProfile: a, sceneTime: o, qualityProfile: s, rock: c, sisyphus: l, dustParticles: u, drawRock: d, drawSisyphus: f } = t, p = {
			sky: () => A(e, n, i, a, o),
			distant: () => te(e, n, r, i),
			mountain: () => {
				j(e), M(e);
			...
			gameplay: () => {},
			fx: () => {
				N(e, c, 32, 9, .28, a), N(e, l, 16, 6, .18, a), d(e), f(e), F(e, u);
			},
			grade: () => {
				P(e, a, s);
			}
...
function te({ createSeededRandom: e }) {
	...
	function n(t) {
		...
		o.addColorStop(0, "#7a8d9e"), o.addColorStop(.2, "#657a8c"), o.addColorStop(.45, "#4d5e6c"), o.addColorStop(.7, "#3a4854"), o.addColorStop(1, "#1e2830"), i.fillStyle = o, i.beginPath(), i.arc(t, t, t, 0, A), i.fill();
		for (let e = 0; e < 18; e++) {
			let e = a() * A, n = a() * t * .8, r = t + Math.cos(e) * n, o = t + Math.sin(e) * n, s = 6 + a() * 14, c = a() > .4, l = i.createRadialGradient(r, o, 0, r, o, s);
			c ? (l.addColorStop(0, `rgba(18, 24, 32, ${(.08 + a() * .12).toFixed(2)})`), l.addColorStop(1, "rgba(18, 24, 32, 0)")) : (l.addColorStop(0, `rgba(140, 160, 180, ${(.04 + a() * .06).toFixed(2)})`), l.addColorStop(1, "rgba(140, 160, 180, 0)")), i.fillStyle = l, i.beginPath(), i.arc(r, o, s, 0, A), i.fill();
		}
		i.save(), i.beginPath(), i.arc(t, t, t, 0, A), i.clip();
		for (let e = 0; e < 14; e++) {
			let n = a() * A, r = a() * t * .85, o = t + Math.cos(n) * r, s = t + Math.sin(n) * r, c = 10 + a() * t * .9, l = n + (a() - .5) * 2.4, u = o + Math.cos(l) * c, d = s + Math.sin(l) * c, f = e % 3 == 0;
		...
		let s = Math.round(t * t * 2.2);
		for (let e = 0; e < s; e++) {
			let e = a() * A, n = Math.sqrt(a()) * t * .97, r = t + Math.cos(e) * n, o = t + Math.sin(e) * n;
			i.fillStyle = a() > .55 ? `rgba(190, 210, 230, ${(.02 + a() * .04).toFixed(2)})` : `rgba(8, 12, 20, ${(.04 + a() * .07).toFixed(2)})`, i.fillRect(r, o, 1, 1);
		}
		let c = i.createRadialGradient(t, t, t * .55, t, t, t);
		return c.addColorStop(0, "rgba(0, 0, 0, 0)"), c.addColorStop(.7, "rgba(0, 0, 0, 0)"), c.addColorStop(1, "rgba(0, 0, 0, 0.3)"), i.fillStyle = c, i.beginPath(), i.arc(t, t, t, 0, A), i.fill(), i.restore(), r;
	}
	function r(e, r, i) {
		let a = i;
		t ||= n(a);
		let o = r.position.x, s = r.position.y, c = r.angle;
		e.save(), e.translate(o, s), e.save(), e.shadowColor = "rgba(4, 8, 16, 0.65)", e.shadowBlur = 14, e.shadowOffsetX = 4, e.shadowOffsetY = 5, e.beginPath(), e.arc(0, 0, a, 0, A), e.fillStyle = "#1e2830", e.fill(), e.restore(), e.save(), e.beginPath(), e.arc(0, 0, a, 0, A), e.clip(), e.rotate(c), e.drawImage(t, -a, -a, a * 2, a * 2), e.rotate(-c);
		let l = e.createRadialGradient(0, 0, a * .15, 0, 0, a);
		l.addColorStop(0, "rgba(0, 0, 0, 0)"), l.addColorStop(.5, "rgba(0, 0, 0, 0)"), l.addColorStop(.8, "rgba(0, 0, 0, 0.25)"), l.addColorStop(1, "rgba(0, 0, 0, 0.6)"), e.fillStyle = l, e.fillRect(-a, -a, a * 2, a * 2);
		let u = e.createLinearGradient(0, -a, 0, a);
		u.addColorStop(0, "rgba(0, 0, 0, 0)"), u.addColorStop(.6, "rgba(0, 0, 0, 0)"), u.addColorStop(1, "rgba(0, 0, 0, 0.35)"), e.fillStyle = u, e.fillRect(-a, -a, a * 2, a * 2);
		let d = -a * .42, f = -a * .48, p = e.createRadialGradient(d, f, 0, d, f, a * 1.15);
		p.addColorStop(0, "rgba(210, 230, 248, 0.28)"), p.addColorStop(.25, "rgba(200, 225, 248, 0.14)"), p.addColorStop(.55, "rgba(200, 225, 248, 0.04)"), p.addColorStop(1, "rgba(200, 225, 248, 0)"), e.fillStyle = p, e.fillRect(-a, -a, a * 2, a * 2);
		let m = -a * .32, h = -a * .36, g = e.createRadialGradient(m, h, 0, m, h, a * .3);
		g.addColorStop(0, "rgba(240, 248, 255, 0.12)"), g.addColorStop(.5, "rgba(220, 235, 248, 0.04)"), g.addColorStop(1, "rgba(220, 235, 248, 0)"), e.fillStyle = g, e.fillRect(-a, -a, a * 2, a * 2), e.restore(), e.beginPath(), e.arc(0, 0, a - .5, 0, A), e.strokeStyle = "rgba(10, 16, 24, 0.2)", e.lineWidth = 1, e.stroke(), e.restore();
	}
	return { drawRock: r };
}
//#endregion
...
var j = Math.PI * 2;
function M({ GAME_STATE: e, PEAK_A_X: t }) {
	function n(e, t) {
		let { sisyphusBody: n, gameState: i, lastGameState: a, stateTransitionTimer: o, STATE_TRANSITION_MAX: s, isPushing: c, sceneTime: u } = t, d = l(n.render.opacity, 0, 1);
		if (!(d <= .005)) if (o > 0 && a) {
			let t = l(o / s, 0, 1), f = 1 - t;
			t > .01 && r(e, a, d * t, n, c, u), f > .01 && r(e, i, d * f, n, c, u);
		} else r(e, i, d, n, c, u);
	}
	function r(t, n, r, l, u, d) {
		if (r <= .005) return;
		t.save(), t.globalAlpha = r, t.translate(l.position.x, l.position.y), t.fillStyle = "#0a0e18", t.strokeStyle = "#0a0e18", t.lineCap = "round", t.lineJoin = "round", t.shadowColor = "rgba(0,0,0,0.5)", t.shadowBlur = 8, t.shadowOffsetY = 3;
		let f = l.position.x;
		n === e.PLAYING ? i(t, f, u, d) : n === e.SUMMIT ? a(t, d) : n === e.SIS_DESCENDING ? o(t, f, d) : s(t, d), t.shadowColor = "transparent", t.shadowBlur = 0, c(t, n, f, r), t.restore();
	}
	function i(e, t, n, r) {
		let i = n ? 1 + .01 * Math.sin(r * .008) : 1;
		e.scale(i, i);
		let a = Math.sin(t * .16), o = n ? Math.sin(r * .02) * 1 : 0;
		e.beginPath(), e.moveTo(-2, 8), e.lineTo(-8 + a * 3, 22), e.lineWidth = 9, e.stroke(), e.beginPath(), e.moveTo(0, 8), e.lineTo(2 - a * 2.5, 22), e.lineWidth = 8, e.stroke(), e.beginPath(), e.moveTo(8, -10), e.bezierCurveTo(4, -2, -1, 4, -3, 9), e.lineWidth = 13, e.stroke(), e.beginPath(), e.arc(9 + o * .2, -14, 6.5, 0, j), e.fill(), e.beginPath(), e.moveTo(6, -6), e.lineTo(12 + o * .3, -12 + o * .15), e.lineWidth = 6, e.stroke(), e.beginPath(), e.moveTo(3, 2), e.lineTo(12 + o * .2, -3 + o * .2), e.lineWidth = 5.5, e.stroke();
	}
	function a(e, t) {
		let n = 1 + .008 * Math.sin(t * .0025);
		e.scale(n, n), e.beginPath(), e.moveTo(-2, 6), e.lineTo(-5, 22), e.lineWidth = 9, e.stroke(), e.beginPath(), e.moveTo(2, 6), e.lineTo(4, 22), e.lineWidth = 9, e.stroke(), e.beginPath(), e.moveTo(0, -12), e.bezierCurveTo(-1, -4, -1, 2, 0, 7), e.lineWidth = 12, e.stroke(), e.beginPath(), e.arc(-1, -17, 6.5, 0, j), e.fill(), e.beginPath(), e.moveTo(-1, -6), e.lineTo(-10, 9), e.lineWidth = 5.5, e.stroke(), e.beginPath(), e.moveTo(1, -6), e.lineTo(8, 8), e.lineWidth = 5, e.stroke();
	}
	function o(e, t, n) {
		let r = Math.sin(t * .09), i = r * 5;
		e.beginPath(), e.moveTo(-3, 9), e.lineTo(-7 + r * 3.5, 22), e.lineWidth = 9, e.stroke(), e.beginPath(), e.moveTo(0, 9), e.lineTo(3 - r * 3.5, 22), e.lineWidth = 8, e.stroke(), e.beginPath(), e.moveTo(1, -8), e.bezierCurveTo(-1, -1, -3, 4, -4, 10), e.lineWidth = 12, e.stroke(), e.beginPath(), e.arc(0, -13, 6.5, 0, j), e.fill(), e.beginPath(), e.moveTo(-1, -2), e.lineTo(-8 + i, 10 + i * .5), e.lineWidth = 5.5, e.stroke(), e.beginPath(), e.moveTo(0, -2), e.lineTo(6 - i, 8 - i * .5), e.lineWidth = 5, e.stroke();
	}
	function s(e, t) {
		let n = 1 + .006 * Math.sin(t * .0018);
		e.scale(n, n), e.beginPath(), e.moveTo(-2, 7), e.lineTo(-6, 16), e.lineWidth = 9, e.stroke(), e.beginPath(), e.moveTo(-6, 16), e.lineTo(-3, 22), e.lineWidth = 7, e.stroke(), e.beginPath(), e.moveTo(0, 7), e.lineTo(4, 16), e.lineWidth = 9, e.stroke(), e.beginPath(), e.moveTo(4, 16), e.lineTo(5, 22), e.lineWidth = 7, e.stroke(), e.beginPath(), e.moveTo(1, -10), e.bezierCurveTo(0, -3, -1, 2, -1, 8), e.lineWidth = 12, e.stroke(), e.beginPath(), e.arc(3, -15, 6.5, 0, j), e.fill(), e.beginPath(), e.moveTo(2, -4), e.lineTo(6, 8), e.lineWidth = 5, e.stroke(), e.beginPath(), e.moveTo(-1, -4), e.lineTo(-5, 7), e.lineWidth = 5, e.stroke();
	}
	function c(n, r, i, a) {
		let o = (.05 + l(i / t, .08, 1) * .14) * a;
		n.save(), n.globalCompositeOperation = "screen", n.strokeStyle = `rgba(148,202,248,${o.toFixed(3)})`, n.lineCap = "round", r === e.PLAYING ? (n.beginPath(), n.moveTo(8, -10), n.bezierCurveTo(4, -2, -1, 4, -3, 9), n.lineWidth = 3, n.stroke()) : r === e.SUMMIT ? (n.beginPath(), n.moveTo(0, -12), n.bezierCurveTo(-1, -4, -1, 2, 0, 7), n.lineWidth = 3, n.stroke()) : r === e.SIS_DESCENDING ? (n.beginPath(), n.moveTo(1, -8), n.bezierCurveTo(-1, -1, -3, 4, -4, 10), n.lineWidth = 3, n.stroke()) : (n.beginPath(), n.moveTo(1, -10), n.bezierCurveTo(0, -3, -1, 2, -1, 8), n.lineWidth = 2.5, n.stroke()), n.restore();
	}
...
function ne(e) {
	...
	for (let t of L) if (!(t in e)) throw TypeError(`VisualLayerProfile missing key "${t}".`);
...
var R = Object.freeze({
	...
	[R.HIGH]: Object.freeze({
		fogAlphaMul: 1,
		grainDensityMul: 1,
		grainAlphaMul: 1,
		overlayOpacityMul: 1
	}),
	[R.MEDIUM]: Object.freeze({
		fogAlphaMul: .86,
		grainDensityMul: .74,
		grainAlphaMul: .82,
		overlayOpacityMul: .9
...
function ce(e = {}) {
	let t = !!e.isMobile, n = {
		tier: e.initialTier || (t ? R.MEDIUM : R.HIGH),
	...
	function a() {
		let e = se(n.tier);
		e < ae.length - 1 && (n.tier = ae[e + 1], n.lastTierChangeAt = performance.now());
	}
	function o() {
		let e = se(n.tier);
		e > 0 && (n.tier = ae[e - 1], n.lastTierChangeAt = performance.now());
	}
	function s(e, t = !1) {
		oe[e] && (n.tier = e, n.badMs = 0, n.goodMs = 0, n.lastTierChangeAt = performance.now(), n.forcedTier = t ? e : null);
	}
	function c() {
		n.forcedTier = null, n.lastTierChangeAt = performance.now();
	}
	function u() {
		let e = ae[(se(n.tier) + 1) % ae.length];
		return s(e, !0), e;
	}
	function d(e) {
						...
						if (l) {
							...
							for (b = 0; b < g.length; b++) {
								...
								for (x = b + 1; x < g.length; x++) {
									var ee = g[x];
									if (s.overlaps(k.bounds, ee.bounds)) {
										var A = k.vertices, te = ee.vertices;
										for (S = 0; S < k.vertices.length; S++) for (w = 0; w < ee.vertices.length; w++) {
											var j = c.magnitudeSquared(c.sub(A[(S + 1) % A.length], te[w])), M = c.magnitudeSquared(c.sub(A[S], te[(w + 1) % te.length]));
											j < O && M < O && (A[S].isInternal = !0, te[w].isInternal = !0);
										}
									}
								}
							}
					...
					r._restingThresh = 2, r._restingThreshTangent = Math.sqrt(6), r._positionDampen = .9, r._positionWarming = .8, r._frictionNormalMultiplier = 5, r._frictionMaxStatic = Number.MAX_VALUE, r.preSolvePosition = function(e) {
						...
						for (p = 0; p < u; p++) {
							...
							if (!(!h.isActive || h.isSensor)) {
								var g = h.collision, _ = g.parentA, v = g.parentB, y = g.normal.x, b = g.normal.y, x = g.tangent.x, S = g.tangent.y, C = h.inverseMass, w = h.friction * h.frictionStatic * c, T = h.contacts, E = h.contactCount, D = 1 / E, O = _.position.x - _.positionPrev.x, k = _.position.y - _.positionPrev.y, ee = _.angle - _.anglePrev, A = v.position.x - v.positionPrev.x, te = v.position.y - v.positionPrev.y, j = v.angle - v.anglePrev;
								for (m = 0; m < E; m++) {
									var M = T[m], N = M.vertex, P = N.x - _.position.x, F = N.y - _.position.y, I = N.x - v.position.x, L = N.y - v.position.y, ne = O - F * ee, re = k + P * ee, ie = A - L * j, R = te + I * j, ae = ne - ie, oe = re - R, se = y * ae + b * oe, ce = x * ae + S * oe, le = h.separation + se, z = Math.min(le, 1);
									z = le < 0 ? 0 : z;
									var ue = z * w;
									ce < -ue || ce > ue ? (f = ce > 0 ? ce : -ce, d = h.friction * (ce > 0 ? 1 : -1) * i, d < -f ? d = -f : d > f && (d = f)) : (d = ce, f = l);
									var de = P * b - F * y, B = I * b - L * y, fe = D / (C + _.inverseInertia * de * de + v.inverseInertia * B * B), V = (1 + h.restitution) * se * fe;
									if (d *= fe, se < o) M.normalImpulse = 0;
									else {
										var H = M.normalImpulse;
										M.normalImpulse += V, M.normalImpulse > 0 && (M.normalImpulse = 0), V = M.normalImpulse - H;
									}
									if (ce < -s || ce > s) M.tangentImpulse = 0;
									else {
										var U = M.tangentImpulse;
										M.tangentImpulse += d, M.tangentImpulse < -f && (M.tangentImpulse = -f), M.tangentImpulse > f && (M.tangentImpulse = f), d = M.tangentImpulse - U;
									}
									var W = y * V + x * d, G = b * V + S * d;
									_.isStatic || _.isSleeping || (_.positionPrev.x += W * _.inverseMass, _.positionPrev.y += G * _.inverseMass, _.anglePrev += (P * G - F * W) * _.inverseInertia), v.isStatic || v.isSleeping || (v.positionPrev.x -= W * v.inverseMass, v.positionPrev.y -= G * v.inverseMass, v.anglePrev -= (I * G - L * W) * v.inverseInertia);
								}
							}
						}
					};
...
function z(e) {
	let { Engine: t, Render: n, Runner: r, Bodies: i, Body: a, Composite: o, Events: s, Query: c, Sleeping: l, Vertices: u } = le.default, d = e.clientWidth || window.innerWidth, f = e.clientHeight || window.innerHeight, h = 1e3, g = 1020, _ = Math.hypot(700, 312);
	700 / _, -(312 / _);
	let b = Math.hypot(20, 80);
	20 / b, 80 / b;
	let S = .65, C = {
		bg: "transparent",
		rock: "#c79a68",
	...
	let A = ee(20260315), j = [], N = 0, L = 0, R = 0, ae = 0, oe = 0, se = 0, z = Object.freeze({
		PLAYING: "playing",
		SUMMIT: "summit",
		SIS_DESCENDING: "sis_descending",
		WAITING: "waiting"
	}), ue = null, de = 0, B = Object.freeze({
		OVERVIEW: "overview",
		FREE_PAN: "free_pan",
		FOCUS_SISYPHUS: "focus_sisyphus"
	}), fe = Object.freeze({
		OVERVIEW: B.OVERVIEW,
		FOCUS_SISYPHUS: B.FOCUS_SISYPHUS
	}), V = {
		behavior: B.OVERVIEW,
	...
	})), _e = v(), ve = 0, ye = P.SOBRIO, be = 0, xe = 0, Se = 0, Ce = 60, we = ce({ isMobile: /Mobi|Android|iPhone|iPad/i.test(navigator.userAgent) }), K = Object.freeze({
		FAR: "far",
		MID: "mid",
		CLOSE: "close"
	}), Te = { level: K.FAR }, Ee = Object.freeze({ DEFAULT: "default" }), De = Object.freeze({ [Ee.DEFAULT]: Object.freeze({
	...
	function ke(e) {
		...
		if (e === B.FOCUS_SISYPHUS) {
			Xe(fe.FOCUS_SISYPHUS);
			return;
		}
		V.behavior = e;
	}
	...
	function Me(e) {
		let t = je(e);
		return t[Te.level] || t[K.FAR];
	}
	function Ne(e) {
		let t = je(e), n = Object.values(t);
		return {
			min: Math.min(...n) - .06,
			max: Math.max(...n) + .18
		};
	...
	function Ie() {
		Fe(), ke(B.FREE_PAN);
		let e = Me(window.innerHeight > window.innerWidth);
		ze(H.x, H.y, e), se = performance.now() + 1200, console.log(`%cZoom activo: ${Pe(Te.level)}`, "color:#a2d2ff;font-weight:bold");
	}
	function Le(e, t = G) {
		let n = Math.max(e, .001), r = d / n, i = f / n, a = t.minX + r / 2, o = t.maxX - r / 2, s = t.minY + i / 2, c = t.maxY - i / 2;
		if (a > o) {
	...
	function ze(e, t, n) {
		let r = Re(e, t, n, G);
		return V.freePanX = r.x, V.freePanY = r.y, r;
	}
	function Be(e, t) {
		W.active = !0, W.moved = !1, W.startClientX = e, W.startClientY = t, W.startCameraX = H.x, W.startCameraY = H.y;
	}
	function Ve() {
		W.active = !1, W.moved = !1;
	}
	function He() {
		return D((Z.position.x - sn) / Math.max(1, on - sn), 0, 1);
	}
	function Ue() {
		...
		return {
			sisCenter: e,
			rockCenter: t,
			portraitView: window.innerHeight > window.innerWidth,
			duoCenterX: (e.x + t.x) / 2,
			duoCenterY: (e.y + t.y) / 2,
			distanceX: Math.abs(t.x - e.x),
			distanceY: Math.abs(t.y - e.y),
			ascentProgress: He(),
			duoSpeed: Math.max(Ut(Q), Ut(Z)),
			leadX: D(Q.velocity.x * 68 + Z.velocity.x * 34, -90, 110),
			leadY: D(Q.velocity.y * 24 + Z.velocity.y * 16, -54, 62)
		};
	}
	function We(e) {
		let t = Me(e.portraitView);
		return $ === z.WAITING ? {
			x: sn + 100,
			y: 370,
			zoom: t
	...
	let Ke = T({
		...
		ne({
			id: ie(P.EXPRESIONISTA, F.WAITING),
			style: P.EXPRESIONISTA,
			mood: F.WAITING,
			stack: I,
			grade: {
				washAlpha: .11,
		...
		ne({
			id: ie(P.EXPRESIONISTA, F.ASCENT),
			style: P.EXPRESIONISTA,
			mood: F.ASCENT,
			stack: I,
			grade: {
		...
		ne({
			id: ie(P.EXPRESIONISTA, F.IMPACT),
			style: P.EXPRESIONISTA,
			mood: F.IMPACT,
			stack: I,
			grade: {
		...
		ne({
			id: ie(P.EXPRESIONISTA, F.DESCENT),
			style: P.EXPRESIONISTA,
			mood: F.DESCENT,
			stack: I,
			grade: {
	...
	], ie(P.SOBRIO, F.ASCENT));
	function Ye(e) {
		return Ke.get(e);
	}
	function Xe(e) {
		let t = Ye(e);
		V.presetId = t.id, V.behavior = t.behavior;
	}
	function Ze() {
		return Ye(V.presetId);
	}
	function Qe() {
		return qe[$] || null;
	}
	...
	function et(e) {
		if (V.behavior === B.FREE_PAN) return G;
		let t = $e();
		return t.boundsResolver ? t.boundsResolver(e) : G;
	}
	function tt() {
		return V.behavior === B.FREE_PAN ? pe : $e().visualProfile || pe;
	}
	function nt() {
		return $ === z.SUMMIT ? F.IMPACT : $ === z.SIS_DESCENDING ? F.DESCENT : $ === z.WAITING ? F.WAITING : Se > .2 ? F.IMPACT : F.ASCENT;
	}
	function rt() {
		let e = nt();
		return Je.getByStyleAndMood(ye, e);
	}
	function it(e) {
		let t = Ne(e.portraitView), n = Qe();
		if (n) {
		...
		if (V.behavior === B.FREE_PAN) return t;
		let r = $e(), i = Math.max(t.min, r.zoomMinResolver(e)), a = Math.min(t.max, r.zoomMaxResolver(e));
		return {
			min: i,
			max: Math.max(i + .02, a)
		};
	...
	function ot(e) {
		if (Oe === e) return;
		let t = De[e] || De[Ee.DEFAULT];
		Oe = e;
		for (let e of [J, vt]) e.style.transformOrigin = t.transformOrigin, e.style.transform = t.transform;
	}
	function st() {
		Ie();
	}
	function ct() {
		ye = ye === P.SOBRIO ? P.EXPRESIONISTA : P.SOBRIO, be = performance.now() + 1400, console.log(`%cEstilo visual: ${ye}`, "color:#f9d29d;font-weight:bold");
	}
	function lt(e) {
		let t = Ut(Z);
		return e.rockCenter.x >= 932 ? {
			...
			x: e.rockCenter.x + D(Z.velocity.x * 62, -120, 60),
			y: e.rockCenter.y - 118 + D(Z.velocity.y * 18, -48, 42),
			zoom: D((e.portraitView ? .88 : .78) - t * .03, .58, .94)
		};
	}
	function ut(e) {
		let t = O(sn + 70, 820, e.sisCenter.x);
		return {
			x: Mt(sn + 96, e.sisCenter.x + 42, t),
			y: Mt(368, e.sisCenter.y - 110, t),
			zoom: Mt(e.portraitView ? .92 : .84, e.portraitView ? 1 : .9, t)
		};
	}
	function dt() {
		let e = Ue();
		if (V.behavior === B.FREE_PAN) {
			let t = Ae(e.portraitView), n = ze(V.freePanX, V.freePanY, t);
			U.x = n.x, U.y = n.y, U.zoom = t;
		} else {
			let t = $e();
			Object.assign(U, t.targetResolver(e));
		}
		let t = it(e);
		U.zoom = D(U.zoom, t.min, t.max);
		let n = Re(U.x, U.y, U.zoom, et(e));
		U.x = n.x, U.y = n.y, ot(at());
	}
	function ft() {
		return W.active ? .34 : V.behavior === B.FREE_PAN ? .22 : $e().lerpSpeed;
	}
	function pt(e) {
		let t = e / 16.6667, r = 1 - (1 - ft()) ** t;
		H.x += (U.x - H.x) * r, H.y += (U.y - H.y) * r, H.zoom += (U.zoom - H.zoom) * r;
		let i = 16 * L * L, a = N * .018, o = Math.sin(a * 1.7 + .6) * i, s = Math.cos(a * 2.1 + 1.1) * i * .68, c = d / H.zoom, l = f / H.zoom;
		n.lookAt(X, {
			min: {
				x: H.x + o - c / 2,
				y: H.y + s - l / 2
	...
	let q = Object.freeze({
		...
		SISYPHUS: 2,
		ROCK: 4
	}), mt = q.SISYPHUS | q.ROCK, ht = q.TERRAIN | q.ROCK, gt = q.TERRAIN | q.SISYPHUS, _t = q.TERRAIN, J = document.createElement("canvas");
	...
	let vt = document.createElement("canvas");
	vt.id = "game-canvas", vt.style.cssText = "position:absolute;inset:0;display:block;width:100%;height:100%;z-index:1;background:transparent;", e.appendChild(vt);
	let yt = J.getContext("2d");
	async function bt() {
		try {
			await y(_e, new URL("data:application/json;base64,", "" + import.meta.url)), console.log("%cOverlays externos cargados", "color:#cfe8b3;font-weight:bold");
		} catch (e) {
			console.warn("No se pudo cargar el manifiesto de overlays externos.", e);
		}
	}
	for (let e of [J, vt]) e.style.transition = "transform 360ms cubic-bezier(0.22, 1, 0.36, 1)", e.style.transform = "translate3d(0, 0, 0) scaleX(1) scaleY(1)", e.style.transformOrigin = "50% 50%";
	let Y = t.create({ gravity: {
		x: 0,
		y: 1
	} }), X = n.create({
		canvas: vt,
	...
	}), xt = i.rectangle(900 / 2, 540, 1020, 40, {
		isStatic: !0,
		collisionFilter: {
			category: q.TERRAIN,
			mask: mt
		...
		render: { visible: !1 },
		label: "ground"
	}), St = i.rectangle(-60 / 2, 560 / 2, 60, 560, {
		isStatic: !0,
		restitution: .1,
		friction: .1,
		collisionFilter: {
		...
		render: { visible: !1 },
		label: "wall-left"
	}), Ct = i.rectangle(930, 560 / 2, 60, 560, {
		isStatic: !0,
		collisionFilter: {
			category: q.TERRAIN,
			mask: mt
		...
		render: { visible: !1 },
		label: "wall-right"
	}), wt = i.rectangle(900 / 2, -60 / 2, 1020, 60, {
		isStatic: !0,
		collisionFilter: {
			category: q.TERRAIN,
			mask: mt
	...
	], Ot = u.centre(Dt), kt = Dt.map((e) => ({
		x: e.x - Ot.x,
		y: e.y - Ot.y
	})), At = i.fromVertices(Ot.x, Ot.y, [kt], {
		isStatic: !0,
		friction: .1,
		restitution: 0,
		collisionFilter: {
	...
	function Nt(e, t, n, r, i) {
		return r === t ? n : Mt(n, i, (e - t) / (r - t));
	}
	function Pt(e) {
		let t = Et[0];
		if (e <= t.x) return t.y;
		for (let t = 1; t < Et.length; t += 1) {
			let n = Et[t - 1], r = Et[t];
			if (e <= r.x) return Nt(e, n.x, n.y, r.x, r.y);
		}
		return Et[Et.length - 1].y;
	}
	function Ft() {
		d = e.clientWidth || window.innerWidth, f = e.clientHeight || window.innerHeight;
		let t = window.devicePixelRatio || 1;
		J.width = d * t, J.height = f * t, J.style.width = `${d}px`, J.style.height = `${f}px`, X.options.width = d, X.options.height = f, X.options.pixelRatio = t, X.canvas.width = d * t, X.canvas.height = f * t, X.canvas.style.width = `${d}px`, X.canvas.style.height = `${f}px`, ze(V.freePanX, V.freePanY, Ae(f > d));
	}
	let It = k({
		LOGICAL_W: 900,
		LOGICAL_H: 560,
		...
		RIGHT_DESCENT_LOW_X: h,
		RIGHT_DESCENT_LOW_Y: 520,
		RIGHT_FACE_BASE_X: g,
		slopeYAt: jt,
		terrainYAt: Pt,
		TERRAIN_SURFACE_POINTS: Et
	}), Lt = te({ createSeededRandom: ee });
	function Rt(e) {
		Lt.drawRock(e, Z, $t);
	}
	let zt = M({
	...
	function Vt() {
		let e = X.bounds.min.x, t = X.bounds.min.y, n = tt(), r = rt();
		yt.save(), yt.setTransform(1, 0, 0, 1, 0, 0), yt.clearRect(0, 0, J.width, J.height);
		let i = window.devicePixelRatio || 1;
		yt.setTransform(i, 0, 0, i, 0, 0), yt.scale(H.zoom, H.zoom), yt.translate(-e, -t), It.drawSceneLayers(yt, {
			viewMinX: e,
			viewMinY: t,
			visualProfile: n,
			layerProfile: r,
			sceneTime: N,
			qualityProfile: we.getProfile(),
			rock: Z,
			sisyphus: Q,
			dustParticles: j,
			drawRock: Rt,
	...
	function Wt(e) {
		a.setVelocity(e, {
			x: 0,
			y: 0
		}), a.setAngularVelocity(e, 0);
	}
	function Gt(e) {
		L = D(L + e, 0, 1);
	}
	function Kt(e, t, n, r = 1) {
		let i = D(n, .08, .6), a = Math.max(3, Math.round(5 + i * 14));
		for (let n = 0; n < a; n += 1) j.push({
			x: e,
			y: t - A() * 10,
			vx: r * (.18 + A() * 1.1) + (A() - .5) * .75,
			vy: -(.3 + A() * 1.5) * i * 2.2,
			radius: 3 + A() * 7 * i,
			alpha: .08 + A() * .14,
			decay: .012 + A() * .02,
	...
	function Jt() {
		...
		if (e - R < 90) return;
		let t = Math.max(Ut(Q), Ut(Z));
		t < .42 || (R = e, Kt(Q.position.x - 6, Pt(Q.position.x) - 2, D(t * .18, .1, .28), -1));
	}
	function Yt() {
		if (!nn || $ !== z.PLAYING) return;
		let e = Y.timing.timestamp;
		if (e - ae < 110) return;
		ae = e;
		let t = Q.position.x - 4, n = Pt(Q.position.x) - 1;
		for (let e = 0; e < 2; e += 1) j.push({
			x: t + (A() - .5) * 5,
			y: n,
			vx: -(.08 + A() * .3),
			vy: -(.04 + A() * .18),
	...
	function Xt() {
		if (!nn || Ut(Q) < .85) return;
		let e = Y.timing.timestamp;
		if (e - oe < 2e3) return;
		oe = e;
		let t = Q.position.x + 7, n = Pt(Q.position.x) - 38;
		j.push({
			x: t + (A() - .5) * 3,
			y: n,
			vx: (A() - .5) * .15,
	...
	function Qt(e) {
		Q.collisionFilter.mask = e ? ht : _t, Z.collisionFilter.mask = e ? gt : _t;
	}
	let $t = 28, en = {
		x: 110,
		y: 491
	}, Z = i.circle(en.x, en.y, 28, {
		restitution: 0,
		friction: .6,
		frictionStatic: 1,
		frictionAir: .006,
	...
	let nn = !1, rn = 0, $ = z.PLAYING, an = 0, on = 830, sn = 188;
	function cn() {
		let e = Z.position.x < 200 && Z.position.y > 480, t = Ut(Z) < .4 && Math.abs(Z.angularVelocity) < .08;
		return e && t;
	}
	function ln(e) {
		if ($ !== e) {
			if (ue = $, de = 450, $ = e, e === z.PLAYING) {
				Ve(), a.setStatic(Q, !1), a.setStatic(Z, !1), Qt(!0), an = 0, Ht();
				return;
			}
			if (e === z.SUMMIT) {
				nn = !1, Ve(), vt.style.cursor = "default", Qt(!1), a.setStatic(Q, !1), Wt(Q), a.setStatic(Q, !0), a.setStatic(Z, !1), a.setVelocity(Z, {
					x: -3.5,
					y: -.5
				}), Ht(), console.log("%cTope alcanzado: La roca regresa rodando por su peso", "color:#d4a373;font-weight:bold");
				return;
			}
			if (e === z.SIS_DESCENDING) {
				Ve(), Qt(!1), a.setStatic(Q, !1), Ht(), an = Y.timing.timestamp + 4e3, console.log("%cSisifo desciende lentamente por 4s...", "color:#8d99ae");
				return;
			}
			if (e === z.WAITING) {
				nn = !1, Ve(), ke(B.OVERVIEW), vt.style.cursor = "default", Qt(!1), a.setStatic(Q, !1), Wt(Q);
				...
				a.setPosition(Q, {
					x: e,
					y: 497
				}), a.setAngle(Q, 0), a.setStatic(Q, !0), Wt(Z), Ht(), console.log("%cHaz clic en Sisifo para subir de nuevo", "color:#e07a5f;font-weight:bold");
			}
		}
	}
	let un = E({
		...
		camera: H,
		cameraDrag: W,
		audioCueController: ge,
		runtimeQualityController: we,
		GAME_STATE: z,
		CAMERA_BEHAVIOR: B,
		...
		setGameState: ln,
		getFreePanZoom: Ae,
		setCameraBehavior: ke,
		wakeBodies: Ht,
		beginCameraDrag: Be,
		stopCameraDrag: Ve,
		storeFreePanTarget: ze,
		cycleUserZoomInput: st,
		cycleVisualStyle: ct,
		setQualityHintUntil: (e) => {
			xe = e;
		}
	});
	un.register(), s.on(Y, "collisionStart", (e) => {
		for (let t of e.pairs) {
			...
			let n = t.bodyA.label === "rock" ? t.bodyA : t.bodyB, r = D((Math.hypot(n.velocity.x - e.velocity.x, n.velocity.y - e.velocity.y) - 1.45) / 6, 0, 1);
			if (r < .08) continue;
			let i = Zt(t, n), a = n.velocity.x < 0 ? -1 : 1;
			Kt(i.x, i.y, .16 + r * .32, a), Gt(r * (e.label === "wall-left" ? .18 : .11)), Se = Math.max(Se, r), ge.triggerImpact(r);
		}
	});
	function dn(e) {
		let t = X.canvas.width, n = X.canvas.height, r = rt(), i = we.getSnapshot(), a = i.profile, o = D((r.grade.washAlpha + r.grade.contrastLift * .1) * (.92 + a.fogAlphaMul * .08), .02, .26), s = D(r.grade.vignetteAlpha, .2, .75), c = Math.max(0, Math.round(r.grain.density * a.grainDensityMul)), l = D((r.grain.baseAlpha + ve * r.grain.motionBoost) * a.grainAlphaMul, 0, .16);
		...
		if (d.addColorStop(0, "rgba(0, 0, 0, 0)"), d.addColorStop(.72, `rgba(7, 10, 16, ${(s * .34).toFixed(3)})`), d.addColorStop(1, `rgba(4, 6, 10, ${s.toFixed(3)})`), e.fillStyle = d, e.fillRect(0, 0, t, n), c > 0 && l > 0) {
			e.globalAlpha = l;
			for (let r = 0; r < c; r += 1) {
				let i = (r * 137 + Math.floor(N * .6)) % t, a = (r * 211 + Math.floor(N * .35)) % n, o = 1 + r % 3;
		...
		performance.now() < be && (e.globalAlpha = .9, e.fillStyle = "rgba(18, 22, 30, 0.64)", e.fillRect(24, 66, 290, 34), e.fillStyle = "rgba(246, 231, 203, 0.95)", e.font = "600 14px system-ui, sans-serif", e.textBaseline = "middle", e.fillText(`ESTILO: ${ye.toUpperCase()}`, 36, 83)), performance.now() < xe && (e.globalAlpha = .9, e.fillStyle = "rgba(18, 22, 30, 0.64)", e.fillRect(24, 108, 220, 34), e.fillStyle = "rgba(198, 231, 255, 0.95)", e.font = "600 14px system-ui, sans-serif", e.textBaseline = "middle", e.fillText(`CALIDAD: ${i.tier.toUpperCase()}`, 36, 125)), e.restore();
	}
	s.on(Y, "beforeUpdate", () => {
		let e = Y.timing.lastDelta || 16.6667;
		N += e, L = Math.max(0, L - e * .0011), Se = Math.max(0, Se - e * .0014), de > 0 && (de = Math.max(0, de - e)), qt(e), Ce = we.update(e).fpsSmoothed;
		let t = D(Ut(Z) / 2.2, 0, 1), n = D(Ut(Q) / 2, 0, 1);
		if (ve = Mt(ve, D(Math.max(t, n) + Se * .3, 0, 1), .1), ge.update({
			rockSpeed: Ut(Z),
			ascentProgress: He(),
			isPushing: nn,
			moodImpact: Se
		}), dt(), pt(e), Z.position.x < 500 && Z.position.y > 452 && Z.velocity.y < -.1 && a.setVelocity(Z, {
			x: Z.velocity.x,
			y: Z.velocity.y * .05
		}), Z.position.x < 34 && Z.velocity.x < -.2 && (a.setVelocity(Z, {
			x: .6,
			y: Z.velocity.y
		}), a.setAngularVelocity(Z, .02)), $ !== z.PLAYING && Math.abs(Z.velocity.x) < .3 && Math.abs(Z.angularVelocity) > .05 && a.setAngularVelocity(Z, Z.angularVelocity * .85), $ !== z.PLAYING && Z.position.y > 487 && Z.position.x > 50 && Math.abs(Z.velocity.x) < .15 && (a.setVelocity(Z, {
			x: 0,
			y: Z.velocity.y
		}), a.setAngularVelocity(Z, 0)), $ === z.PLAYING) {
			if (Q.position.x >= 830) {
				ln(z.SUMMIT);
				return;
			}
			if (!nn) return;
			function e(e, t) {
				let n = e.position.x, r = t / 2, i = Pt(n - r), a = Pt(n + r), o = r * 2, s = a - i, c = Math.hypot(o, s);
				return c === 0 ? {
					x: 1,
					y: 0
				} : {
			...
			{
				Ht(), rn = Math.min(rn + 1, 60);
				let e = .3 + rn / 60 * .7;
				a.applyForce(Q, Q.position, {
					x: t.x * S * e,
					y: t.y * S * e
				}), Jt(), Yt(), Xt();
				let r = Q.velocity.x * t.x + Q.velocity.y * t.y, i = Z.velocity.x * n.x + Z.velocity.y * n.y, o = Math.abs(t.y) < .05 ? .9 : 1.5;
				if (r > o) {
					let e = r - o;
					a.setVelocity(Q, {
						x: Q.velocity.x - t.x * e,
						y: Q.velocity.y - t.y * e
					}), r = o;
				}
				if (r > 0 && i > r * .95) {
					let e = i - r * .95;
					a.setVelocity(Z, {
						x: Z.velocity.x - n.x * e,
						y: Z.velocity.y - n.y * e
					});
				}
			}
		} else $ === z.SUMMIT ? cn() ? an === 0 ? (an = Y.timing.timestamp + 3e3, console.log("%c⏳ Roca en base. Sísifo contempla 3s...", "color:#8d99ae")) : Y.timing.timestamp >= an && (Q.render.opacity = Math.max(0, Q.render.opacity - .033), Q.render.opacity <= 0 && (Wt(Z), ln(z.WAITING))) : an = 0 : $ === z.WAITING && (Q.render.opacity < 1 && (Q.render.opacity = Math.min(1, Q.render.opacity + .006)), l.set(Q, !1));
	}), s.on(X, "afterRender", () => {
		let e = X.context;
		e.save(), e.setTransform(1, 0, 0, 1, 0, 0), e.clearRect(0, 0, X.canvas.width, X.canvas.height), e.restore(), Vt(), dn(e);
	});
	let fn = getComputedStyle(e).position;
	(fn === "static" || fn === "") && (e.style.position = "relative"), e.style.overflow = "hidden", Ft(), window.addEventListener("resize", Ft), bt();
	let pn = r.create();
	return n.run(X), r.run(pn, Y), console.log("%c🪨 Sísifo — listo", "color:#e07a5f;font-size:14px;font-weight:bold"), console.log("Haz clic y mantén sobre Sísifo para empujar la roca."), {
		destroy() {
			n.stop(X), r.stop(pn), s.off(X), s.off(Y), typeof un?.destroy == "function" && un.destroy(), window.removeEventListener("resize", Ft), J.remove(), vt.remove(), o.clear(Y.world), t.clear(Y), console.log("%c🪨 Sísifo — destruido", "color:#999;font-size:12px");
		},
		getState: () => $,
		getPerformance: () => ({
			fpsApprox: Ce,
			motionIntensity: ve,
			currentVisualMood: nt(),
			currentVisualStyle: ye,
			qualityTier: we.getSnapshot().tier
		})
	};
}
//#endregion
export { z as createSisyphusGame };
